...
namespace Logistics.ApiCommand.GLCommand
{
    ...
    /// 添加车辆
    /// </summary>
    [SuperCommand.Attribute.Doc(typeof(CMD_AddVehicle), "添加车辆(等待扫码模式)（物流公司才能使用）")]
    ...
    public class CMD_AddVehicleWait : WebCommandBase, SuperCommand.ICommandBase<WebCommandVOBase>
    {
        public TCommandState ExeCommand<TCommandState>(WebCommandVOBase command) where TCommandState : CommandState, new()
        {
            (bool, string, object) vo = BLL.BLL.LC_Vehicle.AddVehicleWait(web);
